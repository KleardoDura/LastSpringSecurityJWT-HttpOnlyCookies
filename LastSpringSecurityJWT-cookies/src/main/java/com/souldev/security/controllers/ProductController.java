...
public class ProductController {
    ...
    public ResponseEntity<Message> save(@Valid @RequestBody Product product, BindingResult bindingResult){
        if(bindingResult.hasErrors())
            return new ResponseEntity<>(new Message("The details entered are incorrect"), HttpStatus.BAD_REQUEST);
        this.productService.create(product);
        return new ResponseEntity<>(new Message("Saved Product"), HttpStatus.OK);
    }   
}
