...
public class AuthController {
    ...
    public ResponseEntity<Object> login(HttpServletResponse httpServletResponse,
        ...
        try {
            ...
            CookieUtil.create(httpServletResponse, cookieName, jwt, false, -1, "localhost");
                return new ResponseEntity<>(new Message("Login successfully"), HttpStatus.OK);
        } catch (Exception e) {
                return new ResponseEntity<>(new Message("Check your credentials"), HttpStatus.BAD_REQUEST);
        }
    }
    ...
    public ResponseEntity<Object> resgister(@Valid @RequestBody NewUser newUser, BindingResult bindingResult) {
       /*
        ...
        //un
        newUser.getRoles().forEach(role -> roles.add(role));
*/

        System.out.println(newUser.getRoles());
        if (bindingResult.hasErrors())
            return new ResponseEntity<>(new Message("Check your details and try again"), HttpStatus.BAD_REQUEST);
        ...
        return new ResponseEntity<>(new Message("Successful registration!"), HttpStatus.CREATED);
    }
    @GetMapping("/details")
    ...
    public ResponseEntity<Message> logOut(HttpServletResponse httpServletResponse){
        ...
        return new ResponseEntity<>(new Message("Closed session"), HttpStatus.OK) ;
    }
